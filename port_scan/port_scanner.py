...
def connScan(tgtHost, tgtPort):      
    recv_msg = ""
                                                                                                             #용어인데 여러개의 스레드가 한 메모리에 접근하려고 할 경우 스레드끼리 동시에 접근이 가능하면 문제가 생기기때문에 한 스레드가 메모리를 점유하고 있는
    try:                                                                                                     #경우 다른 스레드는 그 스레드가 메모리 사용이 끝날 떄 까지 대기하도록 해야함 이를 구현하기 위해서 세마포어 객체를 사용함
        ...
        connSkt.connect((tgtHost.encode('utf-8'), tgtPort))                                                  #지정한 ip와 port를 사용해 서버와 연결 시도
        connSkt.send("hackerman".encode('utf-8'))                                                            #연결이 되었다면 hackerman이라는 메세지를 보냄
        ...
        recv_msg = str(tgtPort) + "/tcp closed"
                                                                                                            #연결이 닫혀있다는 메세지를 내보냄                                                                                         
    finally:                                                                                                #출력이 끝났으면 lock을 해제해 다른 스레드가 모니터를 사용할 수 있게 설정
        connSkt.close()       
        return recv_msg                                                                               #연결에 사용되었던 소켓 객체 역시 닫아줌
